...
interface UserStatisticsDetailed {
  customerName: string
  mobile: string
  emailId: string
  city: string
  cinemaPassTakenCount: number
  eventTicketBookedCount: number
  totalAmountSpentForMovies: number
  totalAmountSpentForEvents: number
}

interface UserStatisticsDetailedResponse {
  status: string
  message: string
  data: UserStatisticsDetailed[]
  meta: {
    totalUsers: number
    period: string
    date?: string
...
export const userApi = {
  ...
  getUserStatisticsDetailed: async (params: {
    period?: 'daily' | 'weekly' | 'monthly' | 'yearly' | 'custom' | 'all'
    date?: string
    startDate?: string
    endDate?: string
    download?: boolean
  } = {}): Promise<UserStatisticsDetailedResponse | Blob> => {
    const queryParams = new URLSearchParams()
    ...
    if (params.download) {
      ...
      const response = await fetch(`${baseUrl}/admin/users/statistics/detailed?${queryParams.toString()}`, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to export user statistics: ${errorText}`)
      }
    ...
    const response = await apiService.get<UserStatisticsDetailedResponse>(`/admin/users/statistics/detailed?${queryParams.toString()}`)
    return response
  },
...
export type { User, UserStats, CreateUserData, UpdateUserData, UserFilters, ApiResponse, PaginatedResponse, UserStatisticsDetailed, UserStatisticsDetailedResponse }
